    ...
    globalclient.cloudapi.listMachineSnapshots(instance, function(err, snapshots) {
        if (err) {
            ...
            if (minage != null && age != null) {
                if (age > minage) {
                    log.info({instanceName: instance.name, instanceUUID: instance.uuid}, "Creating snapshot");
    ...
    globalclient.cloudapi.listMachineSnapshots(instance, function(err, snapshots) {
        if (err) {
            ...
            if (snaps != null && minsnaps != null) {
                if (snaps > minsnaps) {
                    log.info({instanceName: instance.name, instanceUUID: instance.uuid , snapshots: snapshots.length } , "deleting oldest snapshot");
...
triton.createClient({
    ...
    client.cloudapi.listMachines(function(err, insts) {
        ...
        if (err) {
            ...
            insts.forEach(limit(maybecreatesnapshot).to(1).per(1500));
            insts.forEach(limit(maybedeletesnapshot).to(1).per(1500));
        }
    });
});
